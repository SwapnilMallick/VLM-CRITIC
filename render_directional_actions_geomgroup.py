...
Gripper segmentation uses geom masking (RGBA alpha) instead of the simulator
segmentation buffer — no camera_segmentations="instance" required.
...
Each direction is saved to its own subfolder inside --out_dir:
    <out_dir>/
        ...
        ood_state_gripper.png     # gripper-only render of OOD state
        north/
            image.png             # RGB frame after displacement
            image_gripper.png     # second pass: only gripper geoms visible
...
Usage:
    python render_directional_actions_geomgroup.py
    python render_directional_actions_geomgroup.py --camera agentview --dist 0.025
    python render_directional_actions_geomgroup.py --ood ood_state_agentview.npz --dist 0.05
"""
...
# Substrings present in Franka Panda gripper geom names.
GRIPPER_KEYWORDS = ("hand", "finger", "gripper")

# Substrings present in the Lift task cube geom names.
CUBE_KEYWORDS = ("cube",)
...
def render_gripper_and_cube(env, obs_key: str, vflip: bool) -> np.ndarray:
    """Second render pass: show only gripper and cube geoms, hide everything else.

    Sets alpha=0 on all geoms that are neither gripper nor cube, renders, then
    restores geom_rgba to its original state.
    Returns an RGB uint8 image.
    """
    orig_rgba = env.sim.model.geom_rgba.copy()

    for i, name in enumerate(env.sim.model.geom_names):
        is_gripper = any(kw in name for kw in GRIPPER_KEYWORDS)
        is_cube    = any(kw in name for kw in CUBE_KEYWORDS)
        if not (is_gripper or is_cube):
...
def main(args) -> None:
    ...
    # Save the OOD baseline frame and gripper-only render
    restore_ood(env)
    ...
    ood_gripper_frame = render_gripper_and_cube(env, obs_key, vflip)
    cv2.imwrite(
        os.path.join(args.out_dir, "ood_state_gripper.png"),
        cv2.cvtColor(ood_gripper_frame, cv2.COLOR_RGB2BGR),
    )
    ...
    for label, unit_vec in DIRECTIONS:
        ...
        # First pass: full scene render
        obs   = env._get_observations(force_update=True)
        frame = obs[obs_key][::-1] if vflip else obs[obs_key]

        # Second pass: gripper-only render (non-gripper geoms masked via RGBA alpha)
        gripper_frame = render_gripper_and_cube(env, obs_key, vflip)
        ...
        cv2.imwrite(os.path.join(sub, "image_gripper.png"), cv2.cvtColor(gripper_frame, cv2.COLOR_RGB2BGR))
        np.savez(
